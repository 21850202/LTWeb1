  ...
  save.forEach( async function(s){
    ...
    if(s.finalize == false) {  
      ...
      if(s.finalize == true) {
          ...
          req.banks.Money = req.banks.Money +  m ;
          await Email.send( req.currentUser.email,'SAVE',`Tài khoản tiết kiệm ${s.code} đã được tất toán +${m}VNĐ với lãi suất là ${s.interest} VNĐ vào lúc ${time}  `) 
          req.banks.save();
      }
      s.save();
    }
  });
  res.render('AccountSave',{save});
}));
